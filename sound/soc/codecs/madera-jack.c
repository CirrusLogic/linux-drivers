...
//
// Cirrus Logic Madera class codecs jack support
//
// Copyright (C) 2015-2024 Cirrus Logic, Inc. and
...
#include <sound/soc.h>
#include <dt-bindings/sound/madera-jack.h>
...
#define MADERA_JACK_MASK  (SND_JACK_HEADSET | SND_JACK_LINEOUT | SND_JACK_MECHANICAL)
...
static const struct madera_micd_range madera_micd_default_ranges[] = {
	{ .max =  11, .key = SND_JACK_BTN_0 },
	{ .max =  28, .key = SND_JACK_BTN_1 },
	{ .max =  54, .key = SND_JACK_BTN_2 },
	{ .max = 100, .key = SND_JACK_BTN_3 },
	{ .max = 186, .key = SND_JACK_BTN_4 },
	{ .max = 430, .key = SND_JACK_BTN_5 },
};
...
struct madera_jd_state {
	...
	int (*start)(struct madera_priv *);
	void (*restart)(struct madera_priv *);
	int (*reading)(struct madera_priv *, int);
	void (*stop)(struct madera_priv *);

	int (*timeout_ms)(struct madera_priv *);
	void (*timeout)(struct madera_priv *);
};

static const struct madera_jd_state madera_hpdet_left;
static const struct madera_jd_state madera_hpdet_right;
static const struct madera_jd_state madera_micd_button;
static const struct madera_jd_state madera_micd_microphone;
static const struct madera_jd_state madera_micd_adc_mic;
...
static void madera_jack_report(struct madera_priv *info, int which, bool attached)
{
	dev_dbg(info->dev, "Jack report: %d is %s\n",
		which, attached ? "attached" : "removed");

	if (info->jack.jack) {
		if (attached) {
			info->jack.jack_state |= which;
			snd_soc_jack_report(info->jack.jack, info->jack.jack_state,
				info->jack.jack_state);
		} else {
			info->jack.jack_state = 0;
			snd_soc_jack_report(info->jack.jack,
				info->jack.jack_state, MADERA_JACK_MASK |
				info->jack.micd_button_mask);
		}
		dev_dbg(info->dev, "Jack state: 0x%08x\n", info->jack.jack_state);
	}

	return;
}

static enum madera_accdet_mode madera_jds_get_mode(struct madera_priv *info)
{
	if (info->jack.state)
		return info->jack.state->mode;
	else
		return MADERA_ACCDET_MODE_INVALID;
}

static int madera_jds_set_state(struct madera_priv *info,
			 const struct madera_jd_state *new_state)
{
	int ret = 0;

	if (new_state != info->jack.state) {
		if (info->jack.state)
			info->jack.state->stop(info);

		info->jack.state = new_state;

		if (info->jack.state) {
			ret = info->jack.state->start(info);
			if (ret < 0)
				info->jack.state = NULL;
		}
	}
...
static void madera_jds_reading(struct madera_priv *info, int val)
{
	int ret;

	ret = info->jack.state->reading(info, val);

	if (ret == -EAGAIN && info->jack.state->restart)
		info->jack.state->restart(info);
}

static inline bool madera_jds_cancel_timeout(struct madera_priv *info)
{
	return cancel_delayed_work_sync(&info->jack.state_timeout_work);
}

static void madera_jds_start_timeout(struct madera_priv *info)
{
	const struct madera_jd_state *state = info->jack.state;
	...
	if (state->timeout_ms && state->timeout) {
		...
		queue_delayed_work(system_long_wq, &info->jack.state_timeout_work,
				   msecs_to_jiffies(ms));
	}
}
...
{
	struct madera_priv *info =
		container_of(work, struct madera_priv,
			     jack.state_timeout_work.work);

	mutex_lock(&info->jack.lock);

	if (!info->jack.state) {
		dev_warn(info->dev, "Spurious timeout in idle state\n");
	} else if (!info->jack.state->timeout) {
		dev_warn(info->dev, "Spurious timeout state.mode=%d\n",
			 info->jack.state->mode);
	} else {
		info->jack.state->timeout(info);
		madera_jds_start_timeout(info);
	}

	mutex_unlock(&info->jack.lock);
}

static void madera_jack_hp_clamp(struct madera_priv *info, bool clamp)
{
	...
	if (clamp) {
		...
		info->jack.out_shorted[0] = (info->jack.hp_impedance_x100[0] <=
					  info->jack.hpdet_short_x100);

		if (!info->jack.out_shorted[0]) {
			ret = regmap_update_bits(madera->regmap,
						 MADERA_OUTPUT_ENABLES_1,
						 (MADERA_OUT1L_ENA |
						  MADERA_OUT1R_ENA) <<
...
static const char *madera_jack_get_micbias_src(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int bias = info->jack.micd_modes[info->jack.micd_mode].bias;
...
static const char *madera_jack_get_micbias(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int bias = info->jack.micd_modes[info->jack.micd_mode].bias;
...
static void madera_jack_enable_micbias_pin(struct madera_priv *info,
					   const char *widget)
{
	struct madera *madera = info->madera;
...
static void madera_jack_disable_micbias_pin(struct madera_priv *info,
					    const char *widget)
{
	struct madera *madera = info->madera;
	struct snd_soc_dapm_context *dapm = madera->dapm;
	...
	if (ret)
		dev_warn(info->dev, "Failed to disable %s: %d\n", widget, ret);
...
static void madera_jack_set_micd_bias(struct madera_priv *info, bool enable)
{
	int old_bias = info->jack.micd_bias.bias;
	int new_bias = info->jack.micd_modes[info->jack.micd_mode].bias;
	const char *widget;

	info->jack.micd_bias.bias = new_bias;

	if ((new_bias == old_bias) && (info->jack.micd_bias.enabled == enable))
		return;

	widget = madera_jack_get_micbias_src(info);
	WARN_ON(!widget);

	if (info->jack.micd_bias.enabled)
		madera_jack_disable_micbias_pin(info, widget);

	if (enable)
		madera_jack_enable_micbias_pin(info, widget);

	info->jack.micd_bias.enabled = enable;
}

static void madera_jack_enable_micbias(struct madera_priv *info)
{
	const char *widget = madera_jack_get_micbias(info);

	madera_jack_set_micd_bias(info, true);

	/*
	 * If forced we must manually control the pin state, otherwise
	 * the codec will manage this automatically
	 */
	if (info->jack.pdata->micd_force_micbias)
		madera_jack_enable_micbias_pin(info, widget);
}

static void madera_jack_disable_micbias(struct madera_priv *info)
{
	const char *widget = madera_jack_get_micbias(info);

	madera_jack_set_micd_bias(info, false);
	/*
	 * If forced we must manually control the pin state, otherwise
	 * the codec will manage this automatically
	 */
...
static void madera_jack_set_mode(struct madera_priv *info, int mode)
{
	struct madera *madera = info->madera;

	dev_dbg(info->dev,
		"set mic_mode[%d] src=0x%x gnd=0x%x bias=0x%x gpio=%d hp_gnd=%d\n",
		mode, info->jack.micd_modes[mode].src, info->jack.micd_modes[mode].gnd,
		info->jack.micd_modes[mode].bias, info->jack.micd_modes[mode].gpio,
		info->jack.micd_modes[mode].hp_gnd);

	if (info->jack.micd_pol_gpio)
		gpiod_set_value_cansleep(info->jack.micd_pol_gpio,
					 info->jack.micd_modes[mode].gpio);

	regmap_update_bits(madera->regmap,
			   MADERA_MIC_DETECT_1_CONTROL_1,
			   MADERA_MICD_BIAS_SRC_MASK,
			   info->jack.micd_modes[mode].bias <<
			   MADERA_MICD_BIAS_SRC_SHIFT);
	regmap_update_bits(madera->regmap,
			   MADERA_MIC_DETECT_1_CONTROL_0,
			   MADERA_MICD1_SENSE_MASK,
			   info->jack.micd_modes[mode].src <<
			   MADERA_MICD1_SENSE_SHIFT);
	regmap_update_bits(madera->regmap,
			   MADERA_MIC_DETECT_1_CONTROL_0,
			   MADERA_MICD1_GND_MASK,
			   info->jack.micd_modes[mode].gnd <<
	...
	regmap_update_bits(madera->regmap,
			   ...
			   (8 * (info->jack.pdata->output - 1)),
			   MADERA_HP1_GND_SEL_MASK,
			   info->jack.micd_modes[mode].hp_gnd <<
			   MADERA_HP1_GND_SEL_SHIFT);

	info->jack.micd_mode = mode;
}

static void madera_jack_next_mode(struct madera_priv *info)
{
	int old_mode = info->jack.micd_mode;
	int new_mode;
	bool change_bias = false;

	new_mode = (old_mode + 1) % info->jack.num_micd_modes;

	dev_dbg(info->dev, "change micd mode %d->%d (bias %d->%d)\n",
		old_mode, new_mode,
		info->jack.micd_modes[old_mode].bias,
		info->jack.micd_modes[new_mode].bias);

	if (info->jack.micd_modes[old_mode].bias !=
	    info->jack.micd_modes[new_mode].bias) {
		change_bias = true;

		madera_jack_disable_micbias(info);
	}

	madera_jack_set_mode(info, new_mode);

	if (change_bias)
		madera_jack_enable_micbias(info);
}

static int madera_micd_adc_read(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int val = 0;
	int ret;
...
static int madera_micd_read(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int val = 0;
	int ret, i;
	...
	if (!(val & MADERA_MICD_STS)) {
		...
		lvl = ffs(lvl) - 1;
		if (lvl < info->jack.num_micd_ranges) {
			val = info->jack.micd_ranges[lvl].max;
		} else {
...
static int madera_hpdet_calc_calibration(const struct madera_priv *info,
					 int dacval,
...
static int madera_hpdet_calibrate(struct madera_priv *info,
				  unsigned int range, unsigned int *ohms_x100)
{
	...
	*ohms_x100 = madera_hpdet_calc_calibration(info, dacval,
						   &info->jack.hpdet_trims[range],
						   &info->jack.hpdet_ranges[range]);
	return 0;
}

static int madera_hpdet_read(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	...
	if (range < info->jack.num_hpdet_ranges - 1 &&
	    ((val / 2) >= info->jack.hpdet_ranges[range].max)) {
		range++;
		dev_dbg(info->dev, "Moving to HPDET range %d-%d\n",
			info->jack.hpdet_ranges[range].min,
			info->jack.hpdet_ranges[range].max);
	...
	if (info->jack.hpdet_trims) {
		...
		if (range && ((val / 2) < info->jack.hpdet_ranges[range].min)) {
			dev_dbg(info->dev,
				"Reporting range boundary %d\n",
				info->jack.hpdet_ranges[range].min);
			ohms_x100 =
			      madera_ohm_to_hohm(info->jack.hpdet_ranges[range].min);
		}
	}

	hpdet_ext_res_x100 = info->jack.pdata->hpdet_ext_res_x100;
...
static inline void madera_set_headphone_imp(struct madera_priv *info, int ohms_x100)
{
	info->jack.hp_impedance_x100[0] = ohms_x100;
}

static void madera_hpdet_start_micd(struct madera_priv *info)
{
	struct madera *madera = info->madera;
...
static void madera_hpdet_stop_micd(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int start_time = 1, dbtime = 1, rate = 1;

	if (info->jack.pdata->micd_bias_start_time)
		start_time = info->jack.pdata->micd_bias_start_time;

	if (info->jack.pdata->micd_rate)
		rate = info->jack.pdata->micd_rate;

	if (info->jack.pdata->micd_dbtime)
		dbtime = info->jack.pdata->micd_dbtime;
...
static int madera_hpdet_start(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	int ret;
	...
	if (info->jack.pdata->fixed_hpdet_imp_x100) {
		madera_set_headphone_imp(info,
					 info->jack.pdata->fixed_hpdet_imp_x100);
		ret = -EEXIST;
		goto skip;
	}
	...
	if (info->jack.state->mode == MADERA_ACCDET_MODE_HPL) {
		hpd_clamp = info->jack.pdata->hpd_pins[0];
		hpd_sense = info->jack.pdata->hpd_pins[1];
	} else {
		hpd_clamp = info->jack.pdata->hpd_pins[2];
		hpd_sense = info->jack.pdata->hpd_pins[3];
	}

	hpd_gnd = info->jack.micd_modes[info->jack.micd_mode].gnd;
	...
	madera_jack_hp_clamp(info, false);
...
err:
	madera_jack_hp_clamp(info, true);
...
static int madera_hpdet_wait(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	unsigned int val;
	int i, ret;
...
static void madera_hpdet_stop(struct madera_priv *info)
{
	struct madera *madera = info->madera;
...
static int madera_hpdet_reading(struct madera_priv *info, int val)
{
	...
	madera_jack_report(info, SND_JACK_HEADPHONE, true);

	if (info->jack.have_mic)
...
static int madera_micd_start(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	int ret;
	...
	ret = regulator_enable(info->jack.micvdd);
	if (ret)
		dev_err(info->dev, "Failed to enable MICVDD: %d\n", ret);

	if (info->jack.state->mode == MADERA_ACCDET_MODE_ADC)
...
static void madera_micd_stop(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	...
	regulator_disable(info->jack.micvdd);
...
static void madera_micd_restart(struct madera_priv *info)
{
	struct madera *madera = info->madera;
...
static int madera_micd_button_debounce(struct madera_priv *info, int val)
{
	int debounce_lim = info->jack.pdata->micd_manual_debounce;

	if (debounce_lim) {
		if (info->jack.micd_debounce != val)
			info->jack.micd_count = 0;

		info->jack.micd_debounce = val;
		info->jack.micd_count++;

		if (info->jack.micd_count == debounce_lim) {
			info->jack.micd_count = 0;
			if (val == info->jack.micd_res_old)
				return 0;

			info->jack.micd_res_old = val;
		} else {
			dev_dbg(info->dev, "Software debounce: %d,%x\n",
				info->jack.micd_count, val);
...
static int madera_micd_button_process(struct madera_priv *info, int val)
{
	int i, key;

	if (val < MADERA_MICROPHONE_MIN_OHM) {
		dev_dbg(info->dev, "Mic button detected\n");
		for (i = 0; i < info->jack.num_micd_ranges; i++) {
			if (val <= info->jack.micd_ranges[i].max) {
				key = info->jack.micd_ranges[i].key;
				dev_dbg(info->dev, "Key %d down\n", key);
				snd_soc_jack_report(info->jack.jack, key,
						    info->jack.micd_button_mask);
				break;
			}
		}
		if (i == info->jack.num_micd_ranges)
			dev_warn(info->dev,
				 "Button level %u out of range\n", val);
	} else {
		dev_dbg(info->dev, "Mic button released\n");
		snd_soc_jack_report(info->jack.jack, 0, info->jack.micd_button_mask);
	}

	return 0;
}

static int madera_micd_button_reading(struct madera_priv *info, int val)
{
	int ret;
	unsigned int ohms;
...
static int madera_micd_mic_start(struct madera_priv *info)
{
	int ret;

	info->jack.detecting = true;

	ret = regulator_allow_bypass(info->jack.micvdd, false);
	if (ret)
...
static void madera_micd_mic_stop(struct madera_priv *info)
{
	int ret;

	madera_micd_stop(info);

	ret = regulator_allow_bypass(info->jack.micvdd, true);
	if (ret)
		dev_err(info->dev, "Failed to bypass MICVDD: %d\n", ret);

	info->jack.detecting = false;
}

static int madera_micd_mic_reading(struct madera_priv *info, int val)
{
	unsigned int ohms;
	...
	if (ohms > MADERA_MICROPHONE_MAX_OHM) {
		...
		info->jack.have_mic = info->jack.pdata->micd_open_circuit_declare;
		goto done;
	}
	 ...
	 */
	if (ohms > info->jack.micd_ranges[0].max &&
	    info->jack.num_micd_modes > 1) {
		if (info->jack.jack_flips >= info->jack.num_micd_modes * 10) {
			dev_dbg(info->dev, "Detected HP/line\n");
			goto done;
		} else {
			madera_jack_next_mode(info);

			info->jack.jack_flips++;
	...
	if (info->jack.pdata->hpdet_channel)
		madera_jds_set_state(info, &madera_hpdet_right);
	else
		madera_jds_set_state(info, &madera_hpdet_left);

	madera_jack_report(info, SND_JACK_MICROPHONE, info->jack.have_mic);

	return 0;
}

static int madera_micd_mic_timeout_ms(struct madera_priv *info)
{
	if (info->jack.pdata->micd_timeout_ms)
		return info->jack.pdata->micd_timeout_ms;
	else
		return MADERA_DEFAULT_MICD_TIMEOUT_MS;
}

static void madera_micd_mic_timeout(struct madera_priv *info)
{
	int ret;
	...
	if (ret < 0)
		madera_jack_report(info, SND_JACK_MICROPHONE, false);
}

static int madera_jack_present(struct madera_priv *info,
			       unsigned int *jack_val)
{
	...
	if (info->jack.pdata->jd_use_jd2) {
		val &= MADERA_MICD_CLAMP_RISE_STS1;
		present = 0;
	} else if (info->jack.pdata->jd_invert) {
...
{
	struct madera_priv *info = container_of(work,
						struct madera_priv,
						jack.micd_detect_work.work);
	enum madera_accdet_mode mode;
	int ret;
...
{
	struct madera_priv *info = data;
	int debounce = info->jack.pdata->micd_detect_debounce_ms;

	dev_dbg(info->dev, "micdet IRQ");

	cancel_delayed_work_sync(&info->jack.micd_detect_work);

	mutex_lock(&info->jack.lock);

	if (!info->jack.detecting)
		debounce = 0;
	 ...
	 */
	queue_delayed_work(system_long_wq, &info->jack.micd_detect_work,
...
static irqreturn_t madera_jackdet(int irq, void *data)
{
	...
	if (present) {
		...
		madera_jack_report(info, SND_JACK_MECHANICAL, true);

		info->jack.have_mic = false;
		info->jack.jack_flips = 0;

		if (info->jack.pdata->custom_jd)
			madera_jds_set_state(info, info->jack.pdata->custom_jd);
		else if (info->jack.pdata->micd_software_compare)
		...
		info->jack.have_mic = false;
		info->jack.micd_res_old = 0;
		info->jack.micd_debounce = 0;
		info->jack.micd_count = 0;
		madera_jds_set_state(info, NULL);

		madera_jack_report(info, 0, false);
...
static void madera_jack_of_get_micd_ranges(struct madera_priv *info,
...
static void madera_jack_get_micd_configs(struct madera_priv *info,
					 struct fwnode_handle *node)
{
	...
	info->jack.micd_modes = micd_configs;
	info->jack.num_micd_modes = nconfigs;

err:
	kfree(values);
}

static void madera_jack_get_hpd_pins(struct madera_priv *info,
				     struct fwnode_handle *node,
				     struct madera_accdet_pdata *pdata)
{
	int i, ret;
...
static void madera_jack_process_accdet_node(struct madera_priv *info,
					    struct fwnode_handle *node)
{
	struct madera *madera = info->madera;
	struct madera_accdet_pdata *pdata;
	...
	for (i = 0; i < ARRAY_SIZE(info->jack.accdet); i++)
		if (!info->jack.accdet[i].enabled)
			break;

	if (i == ARRAY_SIZE(info->jack.accdet)) {
		dev_warn(madera->dev, "Too many accdet nodes: %d\n", i + 1);
		return;
	}

	pdata = &info->jack.accdet[i];
	...
	madera_jack_get_hpd_pins(info, node, pdata);
	madera_jack_get_micd_configs(info, node);
	madera_jack_of_get_micd_ranges(info, node, pdata);

	if (info->jack.micd_modes[0].gpio)
		gpio_status = GPIOD_OUT_HIGH;
	else
		gpio_status = GPIOD_OUT_LOW;

	info->jack.micd_pol_gpio = devm_fwnode_gpiod_get_index(info->dev,
							       node,
							       "cirrus,micd-pol",
							       0,
							       gpio_status,
							       "cirrus,micd-pol");
	if (IS_ERR(info->jack.micd_pol_gpio)) {
		if (PTR_ERR(info->jack.micd_pol_gpio) != -ENOENT)
			dev_warn(info->dev,
				 "Malformed cirrus,micd-pol-gpios ignored: %ld\n",
				 PTR_ERR(info->jack.micd_pol_gpio));
		info->jack.micd_pol_gpio = NULL;
	}
}

static int madera_jack_get_device_pdata(struct madera_priv *info)
{
	...
	for_each_child_of_node(parent, child)
		madera_jack_process_accdet_node(info, &child->fwnode);
...
#ifdef DEBUG
#define MADERA_JACK_PDATA_DUMP(x, f) \
	dev_dbg(info->dev, "\t" #x ": " f "\n", pdata->x)

static void madera_jack_dump_config(struct madera_priv *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(info->jack.accdet); ++i) {
		...
		MADERA_JACK_PDATA_DUMP(enabled, "%u");
		MADERA_JACK_PDATA_DUMP(jd_use_jd2, "%u");
		MADERA_JACK_PDATA_DUMP(jd_invert, "%u");
		MADERA_JACK_PDATA_DUMP(fixed_hpdet_imp_x100, "%d");
		MADERA_JACK_PDATA_DUMP(hpdet_ext_res_x100, "%d");
		MADERA_JACK_PDATA_DUMP(hpdet_short_circuit_imp, "%d");
		MADERA_JACK_PDATA_DUMP(hpdet_channel, "%d");
		MADERA_JACK_PDATA_DUMP(micd_detect_debounce_ms, "%d");
		MADERA_JACK_PDATA_DUMP(micd_manual_debounce, "%d");
		MADERA_JACK_PDATA_DUMP(micd_bias_start_time, "%d");
		MADERA_JACK_PDATA_DUMP(micd_rate, "%d");
		MADERA_JACK_PDATA_DUMP(micd_dbtime, "%d");
		MADERA_JACK_PDATA_DUMP(micd_timeout_ms, "%d");
		MADERA_JACK_PDATA_DUMP(micd_clamp_mode, "%u");
		MADERA_JACK_PDATA_DUMP(micd_force_micbias, "%u");
		MADERA_JACK_PDATA_DUMP(micd_open_circuit_declare, "%u");
		MADERA_JACK_PDATA_DUMP(micd_software_compare, "%u");

		if (info->jack.micd_pol_gpio)
			dev_dbg(info->dev, "micd_pol_gpio: %d\n",
				desc_to_gpio(info->jack.micd_pol_gpio));
		else
			dev_dbg(info->dev, "micd_pol_gpio: unused\n");

		dev_dbg(info->dev, "\tmicd_ranges {\n");
		for (j = 0; j < info->jack.num_micd_ranges; ++j)
			dev_dbg(info->dev, "\t\tmax: %d key: %d\n",
				info->jack.micd_ranges[j].max,
				info->jack.micd_ranges[j].key);
		dev_dbg(info->dev, "\t}\n");

		dev_dbg(info->dev, "\tmicd_configs {\n");
		for (j = 0; j < info->jack.num_micd_modes; ++j)
			dev_dbg(info->dev,
				"\t\tsrc: 0x%x gnd: 0x%x bias: %u gpio: %u hp_gnd: %d\n",
				info->jack.micd_modes[j].src,
				info->jack.micd_modes[j].gnd,
				info->jack.micd_modes[j].bias,
				info->jack.micd_modes[j].gpio,
				info->jack.micd_modes[j].hp_gnd);
		dev_dbg(info->dev, "\t}\n");
...
#else
static inline void madera_jack_dump_config(struct madera_priv *info)
{
}
#endif

/* See datasheet for a description of this calibration data */
static int madera_jack_read_calibration(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	...
	ret = regmap_read(madera->regmap_32bit, MADERA_OTP_HPDET_CAL_1, &offset);
	if (ret) {
		dev_err(info->dev,
	...
	ret = regmap_read(madera->regmap_32bit, MADERA_OTP_HPDET_CAL_2, &gradient);
	if (ret) {
		dev_err(info->dev,
			"Failed to read HP CALIB OFFSET value: %d\n", ret);
		return ret;
...
static void madera_jack_set_micd_clamp_mode(struct madera_priv *info)
{
	 ...
	 */
	if (info->jack.pdata->micd_clamp_mode) {
		clamp_ctrl_val = info->jack.pdata->micd_clamp_mode;
	} else if (info->jack.pdata->jd_use_jd2) {
		if (info->jack.pdata->jd_invert)
...
static int madera_jack_add_micd_levels(struct madera_priv *info)
{
	struct madera *madera = info->madera;
	int i, j;
	int ret = 0;
	...
	for (i = 0; i < SND_JACK_SWITCH_TYPES; i++) {
		...
		for (j = 0; j < MADERA_NUM_MICD_BUTTON_LEVELS; j++)
			if (madera_micd_levels[j] >= info->jack.micd_ranges[i].max)
				break;

		if (j == MADERA_NUM_MICD_BUTTON_LEVELS) {
			dev_err(info->dev, "Unsupported MICD level %d\n",
				info->jack.micd_ranges[i].max);
			ret = -EINVAL;
			goto err_input;
		}
		...
		if (info->jack.micd_ranges[i].key > 0) {
			/* SND_JACK_BTN_# masks start with the most significant bit */
			info->jack.micd_button_mask |= SND_JACK_BTN_0 >> i;
			snd_jack_set_key(info->jack.jack->jack, SND_JACK_BTN_0 >> i,
					 info->jack.micd_ranges[i].key);
		}
...
static int madera_jack_init_micd_ranges(struct madera_priv *info)
{
	const struct madera_accdet_pdata *pdata = info->jack.pdata;
	struct madera_micd_range *ranges;
	int i;

	if (pdata->num_micd_ranges == 0) {
		info->jack.micd_ranges = madera_micd_default_ranges;
		info->jack.num_micd_ranges =
	...
	info->jack.micd_ranges = ranges;
	info->jack.num_micd_ranges = pdata->num_micd_ranges;

	for (i = 0; i < pdata->num_micd_ranges - 1; i++) {
		if (info->jack.micd_ranges[i].max > info->jack.micd_ranges[i + 1].max) {
...
int madera_jack_probe(struct madera_priv *info,
		      struct device *dev)
{
	struct madera *madera = info->madera;
	struct madera_accdet_pdata *pdata = &info->jack.accdet[0];
	unsigned int debounce_val;
	int jack_irq_fall, jack_irq_rise;
	int ret, i, hpdet_short_measured;

	info->jack.pdata = pdata;
	info->madera = madera;
	info->dev = dev;
	mutex_init(&info->jack.lock);
	INIT_DELAYED_WORK(&info->jack.micd_detect_work, madera_micd_handler);
	INIT_DELAYED_WORK(&info->jack.state_timeout_work, madera_jds_timeout_work);

	switch (madera->type) {
	case CS47L15:
		info->jack.hpdet_init_range = 1; /* range 0 not used on CS47L15 */
		info->jack.hpdet_ranges = cs47l15_hpdet_ranges;
		info->jack.num_hpdet_ranges = ARRAY_SIZE(cs47l15_hpdet_ranges);
		info->jack.micd_modes = madera_micd_default_modes;
		info->jack.num_micd_modes = ARRAY_SIZE(madera_micd_default_modes);
		break;
	case CS42L92:
	case CS47L92:
	case CS47L93:
		info->jack.hpdet_ranges = cs47l92_hpdet_ranges;
		info->jack.num_hpdet_ranges = ARRAY_SIZE(cs47l92_hpdet_ranges);
		info->jack.micd_modes = madera_micd_default_modes;
		info->jack.num_micd_modes = ARRAY_SIZE(madera_micd_default_modes);
		break;
	default:
		info->jack.hpdet_ranges = madera_hpdet_ranges;
		info->jack.num_hpdet_ranges = ARRAY_SIZE(madera_hpdet_ranges);
		info->jack.micd_modes = madera_micd_default_modes;
		info->jack.num_micd_modes = ARRAY_SIZE(madera_micd_default_modes);
		break;
	}

	ret = madera_jack_get_device_pdata(info);
	if (ret < 0)
		return ret;
	...
	if (hpdet_short_measured < MADERA_HP_SHORT_IMPEDANCE_MIN) {
		...
		dev_warn(info->dev,
			 ...
			 pdata->hpdet_short_circuit_imp,
			 madera_hohm_to_ohm(info->jack.hpdet_short_x100));
	}

	info->jack.micvdd = devm_regulator_get(dev, "MICVDD");
	if (IS_ERR(info->jack.micvdd)) {
		ret = PTR_ERR(info->jack.micvdd);
		dev_err(info->dev, "Failed to get MICVDD: %d\n", ret);
		return ret;
	}

	if (pdata->jd_invert)
		info->jack.last_jackdet =
	...
	if (info->jack.pdata->micd_bias_start_time)
		regmap_update_bits(madera->regmap,
				   MADERA_MIC_DETECT_1_CONTROL_1,
				   MADERA_MICD_BIAS_STARTTIME_MASK,
				   info->jack.pdata->micd_bias_start_time
	...
	if (info->jack.pdata->micd_dbtime)
		regmap_update_bits(madera->regmap,
				   MADERA_MIC_DETECT_1_CONTROL_1,
				   MADERA_MICD_DBTIME_MASK,
				   info->jack.pdata->micd_dbtime
				   << MADERA_MICD_DBTIME_SHIFT);

	ret = madera_jack_init_micd_ranges(info);
	if (ret)
		goto err_input;

	madera_jack_set_micd_clamp_mode(info);

	if ((info->jack.num_micd_modes > 2) && !info->jack.micd_pol_gpio)
		dev_warn(info->dev, "Have >1 mic_configs but no pol_gpio\n");

	madera_jack_set_mode(info, 0);

	pm_runtime_get_sync(madera->dev);

	madera_jack_read_calibration(info);
	...
	for (i = info->jack.hpdet_init_range; i < info->jack.num_hpdet_ranges; ++i) {
		if (madera_ohm_to_hohm(info->jack.hpdet_ranges[i].max) >=
		    pdata->hpdet_ext_res_x100) {
			info->jack.hpdet_init_range = i;
			break;
		}
	}
	if (i == info->jack.num_hpdet_ranges) {
		dev_err(dev,
	...
	if (ret) {
		dev_err(dev, "Failed to get MICDET IRQ: %d\n", ret);
		goto err_input;
	}
	...
	if (ret) {
		dev_err(dev, "Failed to get HPDET IRQ: %d\n", ret);
		goto err_micdet;
	}
	...
	if (ret) {
		dev_err(dev, "Failed to get JD fall IRQ: %d\n", ret);
		goto err_rise_wake;
	}
	...
	madera_jack_dump_config(info);

	info->jack.probed = true;
...
EXPORT_SYMBOL_GPL(madera_jack_probe);
...
void madera_jack_remove(struct madera_priv *info)
{
	...
	cancel_delayed_work_sync(&info->jack.micd_detect_work);
	cancel_delayed_work_sync(&info->jack.state_timeout_work);
...
EXPORT_SYMBOL_GPL(madera_jack_remove);

int madera_jack_set(struct snd_soc_component *component,
		    struct snd_soc_jack *jack, void *data)
{
	struct madera_priv *info = snd_soc_component_get_drvdata(component);
	struct madera *madera = info->madera;
	...
	if (jack) {
		ret = madera_jack_add_micd_levels(info);
		if (ret < 0) {
			info->jack.jack = NULL;
			return -ENODEV;
		}
...
EXPORT_SYMBOL_GPL(madera_jack_set);
