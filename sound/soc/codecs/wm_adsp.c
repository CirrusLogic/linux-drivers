...
int wm_halo_init(struct wm_adsp *dsp, struct mutex *fw_lock, struct mutex *rate_lock)
{
	int ret;
...
out:
	...
	if (ret < 0)
		tstamp->copied_total += wm_adsp_compr_frag_words(compr) *
					WM_ADSP_DATA_WORD_SIZE_DEFAULT;
...
irqreturn_t wm_halo_wdt_expire(int irq, void *data)
{
	...
	if (dsp->compr && dsp->compr->stream) {
		dsp->compr->stream->runtime->state = SNDRV_PCM_STATE_XRUN;
