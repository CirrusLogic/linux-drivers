...
static const struct madera_compr_dai_mapping cs47l85_compr_dai_map[] = {
	{
		.dai_name = "cs47l85-dsp6-voicectrl",
		.dsp_num = 5,
	},
	{
		.dai_name = "cs47l85-dsp1-trace",
		.dsp_num = 0,
	},
};
...
struct cs47l85 {
	...
	struct cs47l85_compr compr_info[ARRAY_SIZE(cs47l85_compr_dai_map)];
};
				 ...
				 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		for (i = 0; i < ARRAY_SIZE(cs47l85->compr_info); ++i) {
			if (cs47l85->compr_info[i].adsp_compr.dsp->num !=
			    w->shift + 1)
				continue;

			mutex_lock(&cs47l85->compr_info[i].trig_lock);
			cs47l85->compr_info[i].trig = false;
			mutex_unlock(&cs47l85->compr_info[i].trig_lock);
		}
...
static void cs47l85_compr_irq(struct cs47l85 *cs47l85,
			      struct cs47l85_compr *compr)
{
...
static irqreturn_t cs47l85_adsp2_irq(int irq, void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(cs47l85->compr_info); ++i) {
		if (!cs47l85->compr_info[i].adsp_compr.dsp->running)
			continue;

		cs47l85_compr_irq(cs47l85, &cs47l85->compr_info[i]);
	}
	return IRQ_HANDLED;
}

static int cs47l85_compr_open(struct snd_compr_stream *stream)
{
	...
	compr_idx = madera_get_compr_map_idx(rtd,
					     cs47l85_compr_dai_map,
					     ARRAY_SIZE(cs47l85_compr_dai_map));
	if (compr_idx < 0) {
	...
	return wm_adsp_compr_open(&cs47l85->compr_info[compr_idx].adsp_compr,
				  stream);
}

static int cs47l85_compr_trigger(struct snd_compr_stream *stream, int cmd)
{
	struct wm_adsp_compr *adsp_compr = stream->runtime->private_data;
	struct cs47l85_compr *compr = container_of(adsp_compr,
						   struct cs47l85_compr,
						   adsp_compr);
...
static void cs47l85_init_compr_info(struct cs47l85 *cs47l85)
{
	struct wm_adsp *dsp;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(cs47l85->compr_info); ++i) {
		cs47l85->compr_info[i].priv = cs47l85;

		cs47l85->compr_info[i].dai_name =
			cs47l85_compr_dai_map[i].dai_name;

		dsp = &cs47l85->core.adsp[cs47l85_compr_dai_map[i].dsp_num],
		wm_adsp_compr_init(dsp, &cs47l85->compr_info[i].adsp_compr);

		mutex_init(&cs47l85->compr_info[i].trig_lock);
	}
}

static void cs47l85_destroy_compr_info(struct cs47l85 *cs47l85)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(cs47l85->compr_info); ++i)
		wm_adsp_compr_destroy(&cs47l85->compr_info[i].adsp_compr);
}

static struct snd_compr_ops cs47l85_compr_ops = {
	.open = cs47l85_compr_open,
	.free = wm_adsp_compr_free,
	.set_params = wm_adsp_compr_set_params,
	.trigger = cs47l85_compr_trigger,
...
static int cs47l85_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = madera_init_dsp_irq(codec, cs47l85_adsp2_irq, cs47l85);
	if (ret)
		return ret;
