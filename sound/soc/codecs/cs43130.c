...
{
	return cs43130_show_dc(dev, buf, CS43130_HPL);
}

static ssize_t cs43130_show_dc_r(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return cs43130_show_dc(dev, buf, CS43130_HPR);
}
...
static ssize_t cs43130_show_ac_l(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return cs43130_show_ac(dev, buf, CS43130_HPL);
}

static ssize_t cs43130_show_ac_r(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return cs43130_show_ac(dev, buf, CS43130_HPR);
}
				 ...
				 struct cs43130_private *cs43130)
{
	...
	if (msk == CS43130_HPLOAD_DC_INT) {
		if (left_ch)
			cs43130->hpload_dc[CS43130_HPL] = impedance;
		else
			cs43130->hpload_dc[CS43130_HPR] = impedance;

		dev_dbg(codec->dev, "HP DC impedance (Ch %u): %u\n", !left_ch,
			impedance);
	} else {
		if (left_ch)
			cs43130->hpload_ac[ac_idx][CS43130_HPL] = impedance;
		else
			cs43130->hpload_ac[ac_idx][CS43130_HPR] = impedance;
...
static void cs43130_imp_meas(struct work_struct *wk)
{
	...
	case CS43131_CHIP_ID:
		...
		break;
	default:
		dev_err(codec->dev, "Wrong device ID.\n");
		return;
	}
	...
	if (cs43130->hpload_dc[CS43130_HPL] >= CS43130_LINEOUT_LOAD)
	...
	cs43130_set_hv(cs43130->regmap, cs43130->hpload_dc[CS43130_HPL],
