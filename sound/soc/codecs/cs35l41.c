		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	unsigned int fw_status = CSPL_MBOX_STS_RUNNING;
	...
	case SND_SOC_DAPM_POST_PMU:
		if (!cs35l41->dsp.running)
			...
			regmap_read(cs35l41->regmap, CS35L41_DSP_MBOX_2, &fw_status);
			switch (fw_status) {
...
static const struct snd_soc_dapm_widget cs35l41_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY_S("DSP1 Preloader", 100,
				SND_SOC_NOPM, 0, 0, cs35l41_dsp_preload_ev,
				SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_OUT_DRV_E("DSP1", SND_SOC_NOPM, 0, 0, NULL, 0,
				cs35l41_dsp_audio_ev, SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD),
		  ...
		  struct cs35l41_platform_data *pdata)
{
	...
	if (ret < 0) {
		dev_err(cs35l41->dev, "dsp_init failed: %d\n", ret);
		goto err;
	}
