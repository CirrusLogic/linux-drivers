		  ...
		  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		return clk_prepare_enable(madera->mclk[clk_idx]);
	case SND_SOC_DAPM_POST_PMD:
		clk_disable_unprepare(madera->mclk[clk_idx]);
		return 0;
	default:
		return 0;
	}
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	val = snd_soc_read(codec, MADERA_INPUT_ENABLES);
...
static int madera_set_opclk(struct snd_soc_codec *codec,
...
static int madera_set_outclk(struct snd_soc_codec *codec,
				 ...
				 struct snd_soc_dai *dai)
{
	...
	snd_soc_update_bits(codec, reg, MADERA_SAMPLE_RATE_1_MASK, sr_val);
...
static int madera_aif_cfg_changed(struct snd_soc_codec *codec,
				   ...
				   unsigned int shift)
{
	...
	case MADERA_FLL_SRC_MCLK1:
		clk = madera->mclk[MADERA_MCLK1];
		break;
	case MADERA_FLL_SRC_MCLK2:
		clk = madera->mclk[MADERA_MCLK2];
		break;
	case MADERA_FLL_SRC_MCLK3:
		clk = madera->mclk[MADERA_MCLK3];
		break;
	default:
		return 0;
	}
