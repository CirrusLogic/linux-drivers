...
int madera_init_spk(struct snd_soc_codec *codec, int n_channels)
{
	...
	ret = snd_soc_dapm_new_controls(&codec->dapm, madera_spk, n_channels);
	if (ret)
		return ret;
...
{
	struct madera_enum *priv_enum =
		(struct madera_enum *)kcontrol->private_value;

	ucontrol->value.enumerated.item[0] = priv_enum->val;

	return 0;
}
EXPORT_SYMBOL_GPL(madera_mux_get);

int madera_mux_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_context *dapm = snd_soc_dapm_kcontrol_dapm(kcontrol);
	struct madera_enum *priv_enum = (struct madera_enum *)kcontrol->private_value;
	struct soc_enum *e = &(priv_enum->mixer_enum);
...
EXPORT_SYMBOL_GPL(madera_mux_put);

int madera_mux_ev(struct snd_soc_dapm_widget *w,
		  struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	priv_enum = (struct madera_enum *)w->kcontrols[0]->private_value;
	e = &(priv_enum->mixer_enum);
	mask = e->mask << e->shift_l;

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		val = snd_soc_enum_item_to_val(e, priv_enum->val);
		val <<= e->shift_l;
	...
	case SND_SOC_DAPM_PRE_PMD:
		mutex_lock(&priv->rate_lock);
		snd_soc_update_bits(codec, e->reg, mask, 0);
		mutex_unlock(&priv->rate_lock);
		return 0;
	case SND_SOC_DAPM_PRE_REG:
		ret = regmap_read(priv->madera->regmap, e->reg, &val);
		if (ret)
			return ret;
		/* If control is not powered-up don't change its setting */
		if (val == 0)
			update->val = 0;
...
static int madera_write_adsp_clk_setting(struct wm_adsp *dsp, unsigned int freq)
{
	...
	case WM1840:
		...
		mask = MADERA_DSP_RATE_MASK | MADERA_DSP_CLK_SEL_MASK;
		val = priv->adsp_rate_cache[dsp->num - 1] <<
			MADERA_DSP_RATE_SHIFT;
		val |= (freq << MADERA_DSP_CLK_SEL_SHIFT);

		dev_dbg(priv->madera->dev,
			"Set legacy DSP clocking to 0x%x\n", val);
	...
	default:
		...
		dev_dbg(priv->madera->dev, "Set DSP frequency to %u\n", freq);
...
static int madera_set_adsp_clk(struct wm_adsp *dsp, unsigned int freq)
{
	...
	cur &= MADERA_DSP_RATE_MASK | MADERA_DSP_CLK_SEL_MASK;
	new = priv->adsp_rate_cache[dsp->num - 1] << MADERA_DSP_RATE_SHIFT;
	new |= (freq << MADERA_DSP_CLK_SEL_SHIFT);
		 ...
		 int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		priv->in_pending--;
		snd_soc_update_bits(w->codec, reg, MADERA_IN1L_MUTE, 0);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		reg = snd_soc_read(w->codec, MADERA_INPUT_ENABLES);
		if (reg == 0)
